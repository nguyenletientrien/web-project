...
function registerUser() {
    ...
    var phone_num = document.querySelector('.auth__form-signin-input-phone-nums').value;
    ...
    if (!username || !name || !email || !phone_num || !password || !confirmPassword) {
    ...
    document.querySelector('.auth__form-sigin-input-user').value = "";
    document.querySelector('.auth__form-signin-input-name').value = "";
    document.querySelector('.auth__form-signin-input-email').value = "";
    document.querySelector('.auth__form-signin-input-phone-nums').value = "";
    document.querySelector('.auth__form-signin-input-password').value = "";
    document.querySelector('.auth__form-signin-input-password-confirm').value = "";
...
function getNextUserCode(existingUsers) {
    ...
    if (existingUsers.length === 0) {
        ...
        if (lastUserCode && /^KN\d{2}$/.test(lastUserCode)) {
            ...
            // console.error('Unexpected user code format. Setting default user code.');
            // newUserCode = 'KN01'; // Set a default user code
        }
    }
...
function generateOrderCode() {
    // This function generates a 4-digit order code starting from 0001 and increments it for each order.
    // You may need to implement a more sophisticated logic for a production environment.
    const lastOrderCode = localStorage.getItem('lastOrderCode') || 0;
    const newOrderCode = parseInt(lastOrderCode) + 1;

    // Save the new order code to localStorage
    localStorage.setItem('lastOrderCode', newOrderCode);

    // Pad the order code with leading zeros
    return newOrderCode.toString().padStart(4, '0');
}
